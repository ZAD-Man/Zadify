...
namespace Zadify.Activities
{
    ...
    public class MonsterDisplay : Activity
    {
        protected override void OnCreate(Bundle bundle)
        {
            ...
            {
                case "Create":
                    monsterText.Text = string.Format("You are locked in a room. You can hear a {0} trying to break in! Complete your goal to survive!", monster.ToLower());
                    break;
                case "Nothing":
                ...
                case "Progress":
                    ...
                    {
                        monsterText.Text = string.Format("You open an small ice box and find {0}! You were starting to get hungry anyway, so you quickly eat it.", food.ToLower());
                    }
                    else if (percentDone >= 60 && percentDone < 90)
                    {
                        monsterText.Text = string.Format("You manage to push a heavy dresser off of a pile of clutter. Underneath it you find a {0}! This will be useful should worst come to worst.", defense.ToLower());
                    }
                    else if (percentDone >= 90 && percentDone < 100)
                    {
                        monsterText.Text = string.Format("You find a safe in the far corner of the room. The door is cracked open, so you pull it open and look inside. There's a {0} in there! You eagerly take it. You feel a lot better with the {0} at your side.", weapon.ToLower());
                    }
                    else if (percentDone >= 100)
                    {
                        monsterText.Text = string.Format("You set up debris in front of the door. You adjust your {0} and ready your {1}. When that {2} comes through the door, you'll be ready.", defense.ToLower(), weapon.ToLower(), monster.ToLower());
                    }
                    break;
                case "Complete":
